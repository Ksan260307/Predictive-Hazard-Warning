...
def test_画面の広くを占める目の前の物は自分の動きと間違えない():
    # 変化が大きくても、1つの塊にまとまっている時は「目の前の大きな物」
    # (バス・直前の歩行者など) として拾う。一番危険な物を取りこぼさない
    finder = MovingThingFinder(stabilize=True)
    finder.find(np.zeros((120, 160, 3), dtype=np.uint8))
    frame = np.zeros((120, 160, 3), dtype=np.uint8)
    frame[10:110, 30:140] = 255  # 画面の半分以上を占める大きな物
    things = finder.find(frame)
    assert len(things) == 1


def test_変化が散らばっている時は大きな塊としては拾わない():
    # 画面のあちこちが同時に変わった (歩いて背景全体が流れた) 時は捨てる
    finder = MovingThingFinder(stabilize=True)
    finder.find(np.zeros((120, 160, 3), dtype=np.uint8))
    frame = np.zeros((120, 160, 3), dtype=np.uint8)
    # 離れた4か所が同時に大きく変わる
    frame[10:60, 10:70] = 255
    frame[10:60, 90:150] = 255
    frame[65:110, 10:70] = 255
    frame[65:110, 90:150] = 255
    things = finder.find(frame)
    assert things == []
